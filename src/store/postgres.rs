...
use super::{
    ...
    MemoryWithEmbedding, MemoryWriter, ReassignScopeOutcome, RecordUseOutcome, ReembedClaim, ReembedClaimScope,
...
impl PostgresStore {
    ...
    pub(crate) async fn store_batch_with_metadata_contexts_audited_impl(
        ...
        for (memory, item_metadata) in memories.iter().zip(metadata) {
            ...
            if memory.memory.provenance.source_agent.as_deref().is_none_or(str::is_empty) {
    ...
    pub(crate) async fn list_with_embeddings_impl(&self, context_ids: Option<&[ContextId]>, limit: usize) -> Result<Vec<MemoryWithEmbedding>, StoreError> {
        if limit == 0 {
            return Ok(Vec::new());
        }
    ...
    pub(crate) async fn update_authorized_if_unmodified_with_metadata_audited_impl(
        ...
        if let Some(context_ids) = context_ids {
            ...
            if get_metadata_tx(&mut tx, id).await?.is_none() {
                upsert_metadata_tx(
                    &mut tx,
                    &MemoryMetadata {
                        memory_id: *id,
...
mod tests {
    ...
    async fn reembed_embeddings_and_neighbors_against_postgres() {
        ...
        let scoped = store.list_with_embeddings_impl(Some(std::slice::from_ref(&context_id)), 10_usize).await.unwrap();
