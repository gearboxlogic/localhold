...
pub(super) fn validate_action_references(path: &str, source: &str) -> Result<()> {
    ...
    for line in source.lines() {
        ...
        if is_audited_local_reference(&reference) || REVIEWED_REMOTE_ACTIONS.contains(&reference.as_str()) {
            continue;
        }
...
fn is_audited_local_reference(reference: &str) -> bool {
    let Some(relative) = reference.strip_prefix("./") else {
        return false;
    };
    !relative.is_empty() && Path::new(relative).components().all(|component| matches!(component, std::path::Component::Normal(_)))
}
