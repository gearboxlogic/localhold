...
fn execution_input_candidates<'a>(surface: ShellSurface<'_>, tokens: &'a [String]) -> (Vec<&'a str>, bool) {
    ...
    if raw_command_word.contains(['*', '?', '[', '{', '~']) && !matches!(raw_command_word, "[" | "[[" | "{") {
        return (Vec::new(), true);
    }
    ...
    dispatch_execution_input(DispatchInput {
        surface,
        tokens,
        command_index,
        raw_command_word,
...
fn dispatch_execution_input<'a>(input: DispatchInput<'a, '_>) -> (Vec<&'a str>, bool) {
    ...
    let selected = match command.as_str() {
        ...
        "tar" | "tar.exe" if program::tar_dispatch_is_opaque(arguments) => SelectedInput::Opaque,
        "zip" | "zip.exe" if program::zip_test_command_is_opaque(arguments) => SelectedInput::Opaque,
        "openssl" | "openssl.exe" if program::openssl_module_selection_is_opaque(arguments) => SelectedInput::Opaque,
        "rg" | "rg.exe" | "ripgrep" | "ripgrep.exe" if program::ripgrep_preprocessor_is_opaque(arguments) => SelectedInput::Opaque,
        "cargo" | "cargo.exe" if cargo::dispatch_is_opaque(arguments) => SelectedInput::Opaque,
        "just" | "just.exe" if program::just_source_selection_is_opaque(arguments) => SelectedInput::Opaque,
        ...
        "git" | "git.exe" => {
            return (Vec::new(), git::dispatch_is_opaque(surface.path, arguments));
        }
