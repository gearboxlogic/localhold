...
fn indirect_command_execution_fails_closed() {
    for command in [
        ...
        "zip --test --unzip-command 'sh quality/lint.txt' archive.zip input.txt",
        "zip.exe --tes --unz=quality/lint.txt archive.zip input.txt",
        "zip archive.zip input.txt --unzip-command=quality/lint.txt --test",
        "openssl list -provider-path quality -provider lint",
        "openssl.exe req -engine lint -new",
        "openssl req -config quality/openssl.cnf",
        "openssl req -config=quality/openssl.cnf",
        "openssl req --config quality/openssl.cnf",
        "openssl.exe req --config=quality/openssl.cnf",
        "openssl req -keygen_engine quality/lint.so -new",
        "openssl req --keygen_engine=quality/lint.so -new",
        "openssl req --engine=lint -new",
        "openssl list --provider=lint",
        "openssl list --provider-path=quality --provider lint",
        ...
        "gcc -specs quality/lint.specs -c quality/input.c",
        "ld.so quality/lint",
        ...
        "sed '\\#foo#e sh quality/lint.txt' /etc/hosts",
        "sed '\\#foo#,/bar/e sh quality/lint.txt' /etc/hosts",
        "sed '\\#foo\\#bar#e sh quality/lint.txt' /etc/hosts",
...
fn more_indirect_command_execution_fails_closed() {
    ...
    for command in [
        ...
        "zip -t archive.zip input.txt",
        "zip --temp-path scratch archive.zip input.txt",
        "zip --testing archive.zip input.txt",
        "zip --unzip-commander=quality/lint.txt archive.zip input.txt",
        "zip archive.zip input.txt -- --test --unzip-command=quality/lint.txt",
        "openssl version",
