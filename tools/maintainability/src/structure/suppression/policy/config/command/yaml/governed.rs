...
const LINUX_SHELL: &str = "/usr/bin/env -u BASH_ENV -u BASHOPTS -u ENV -u GCONV_PATH -u SHELLOPTS -u PERL5LIB -u PERL5OPT -u PERLLIB -u LD_AUDIT -u LD_LIBRARY_PATH -u LD_PRELOAD /usr/bin/bash --noprofile --norc -e -o pipefail {0}";
...
const GATE_ENVIRONMENT: [(&str, &str); 13] = [
    ("BASH_ENV", ""),
    ("BASHOPTS", ""),
    ("GCONV_PATH", ""),
    ("SHELLOPTS", ""),
