...
const LINUX_SHELL: &str =
    "/usr/bin/env -u BASH_ENV -u ENV -u GCONV_PATH -u SHELLOPTS -u LD_AUDIT -u LD_LIBRARY_PATH -u LD_PRELOAD /usr/bin/bash --noprofile --norc -e -o pipefail {0}";
...
const GATE_ENVIRONMENT: [(&str, &str); 9] = [
    ("BASH_ENV", ""),
    ("GCONV_PATH", ""),
    ("SHELLOPTS", ""),
    ("LD_AUDIT", ""),
