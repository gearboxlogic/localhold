...
pub(super) fn type_declaration_evidence(kind: &str, ident: &proc_macro2::Ident, visibility: &Visibility, context: &TypeDeclarationContext<'_>) -> TypeDeclarationEvidence {
    let mut item_path = context.module.to_vec();
    item_path.push(normalized_ident(ident));
    let identity = format!(
        "type-declaration:{kind}:{}\0visibility:{}\0cfg:{}\0ancestors:{}",
        item_path.join("::"),
